...
/- 5 points -/ 
theorem p314 {n : ℤ} (hn : Odd n) : Odd (7 * n - 4) := by
  dsimp [Odd] at *
  obtain ⟨k, hk⟩ := hn 
  use 7 * k + 1 
  rw [hk] 
...
/- 5 points -/ 
theorem p316 {x y : ℤ} (hx : Odd x) (hy : Odd y) : Odd (x * y + 2 * y) := by
  dsimp [Odd] at * 
  obtain ⟨a, ha⟩ := hx
  obtain ⟨b, hb⟩ := hy
  use 2 * a * b + a + 3 * b + 1 
...
/- 5 points -/ 
theorem p318 {n : ℤ} (hn : Even n) : Odd (n ^ 2 + 2 * n - 5) := by
  dsimp [Even] at hn 
  dsimp [Odd] 
  obtain ⟨k, hk⟩ := hn 
  use 2 * k ^ 2 + 2 * k - 3 
...
/- 5 points -/ 
theorem p3114 (a b c : ℤ) : Even (a - b) ∨ Even (a + c) ∨ Even (b - c) := by
...
/- 5 points -/ 
theorem p413 {a b : ℝ} (h : ∀ x, x ≥ a ∨ x ≤ b) : a ≤ b := by
...
/- 5 points -/ 
theorem p416 : ∃ c : ℝ, ∀ x y, x ^ 2 + y ^ 2 ≤ 4 → x + y ≥ c := by
  use -3 
  intros x y h 
  have h1 : (x + y) ^ 2 ≤ 3 ^ 2 := 
  calc 
...
/- 5 points -/ 
theorem p412 {n : ℤ} (hn : ∀ m, 1 ≤ m → m ≤ 5 → m ∣ n) : 15 ∣ n := by
...
/- 5 points -/ 
theorem p414 : forall_sufficiently_large x : ℝ, x ^ 3 + 3 * x ≥ 7 * x ^ 2 + 12 := by
  use 7 
  intros x hx 
  calc 
...
/- 10 points -/ 
theorem p425 {x : ℝ} : x ^ 2 + x - 6 = 0 ↔ x = -3 ∨ x = 2 := by
  constructor 
  · intros h 
...
/- 10 points -/ 
theorem p426 {a : ℤ} : a ^ 2 - 5 * a + 5 ≤ -1 ↔ a = 2 ∨ a = 3 := by
  constructor 
  · intros h 
    have h1 : (2 * a - 5) ^ 2 ≤ 1 ^ 2 := 
    calc 
